...
RESPUESTA: En este caso, al usar una interfaz o clase abstracta, podemos definir métodos comunes que las subclases pueden sobreescribir y así evitamos duplicar código (cosa que ocurre en el ejemplo v0 con el `when`)

c) ¿Qué modificadores y mecanismos has utilizado para **bloquear** y **forzar** la herencia de clases y métodos?

RESPUESTA: `final` para bloquear y `open` para forzar
